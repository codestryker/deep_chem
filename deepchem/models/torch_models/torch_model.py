...
class TorchLitModel(pl.LightningModule):
  ...
  def training_step(self, train_batch, batch_idx):
    ...
    self.log('loss', batch_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
  ...
  def training_step_end(self, batch_parts):
    ...
    self.log('Ending global_step %d: Average loss' % (self._global_step), avg_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
    return {"loss":avg_loss}
  
  def training_epoch_end(self, step_outputs):
       if self.current_epoch==self.nb_epoch-1:
           end_time=time.time()
           self.log("TIMING: model fitting took %0.3f s" % (end_time - self.start_time),prog_bar=False)
...
class TorchModel(Model):
  ...
  def fit(self,
   ...
   """
    ...
    trainer = pl.Trainer(
               ...
               flush_logs_every_n_steps=self.log_frequency,
  ...
  def default_generator(
    ...
    for (X_b, y_b, w_b, ids_b) in dataset.iterbatches(
        ...
        yield self._prepare_batch(([X_b], [y_b], [w_b]))
